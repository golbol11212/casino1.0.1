document.addEventListener('DOMContentLoaded', () => {
    ...
    const caseType = 'silver'; // Тип кейса для этого файла
    const caseCost = 500; // Вартість срібного кейса
    ...
    function updateUI() {
        ...
        openCaseBtn.textContent = `Открыть ${amount} серебряный кейс(-а) (${totalCost} очков)`;
    ...
    function generateSingleItem() {
        const points = Math.floor(Math.random() * 991) + 10;
        let rarity = 'common';
        if (points > 950) rarity = 'legendary';
        else if (points > 800) rarity = 'mythical';
        else if (points > 500) rarity = 'rare';
        else if (points > 200) rarity = 'uncommon';
    ...
    function runSpinAnimation(reelsElement) {
        return new Promise(resolve => {
            ...
            setTimeout(() => {
                const randomSpeed = 4 + Math.random() * 2; // 4-6 seconds
