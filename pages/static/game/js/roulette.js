...
document.getElementById('roulette-app').append(container);
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (initialBalanceElement) {
        bankValue = parseInt(initialBalanceElement.dataset.balance);
    } else {
        bankValue = 1000; // Fallback if element not found
    }
    startGame();
});
...
window.addEventListener('beforeunload', () => {
    updateBalanceOnServer(bankValue);
});
...
function resetGame(){
	...
	updateBalanceOnServer(bankValue); // Обновляем баланс на сервере
	currentBet = 0;
	wager = 5;
	bet = [];
	numbersBet = [];
...
async function updateBalanceOnServer(newBalance) {
    try {
        const response = await fetch('/game/update_roulette_balance/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Функция getCookie должна быть определена
            },
        ...
        if (data.status === 'success') {
            ...
            if (headerBalanceElement) {
                ...
                console.log('Баланс в шапке обновлен до:', headerBalanceElement.innerText);
            } else {
                console.warn('Элемент .balance-amount не найден в DOM.');
            }
        } else {
